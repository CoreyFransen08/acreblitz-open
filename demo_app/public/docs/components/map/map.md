...
| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `clickForecast` | `ClickForecastOptions` | - | Click-to-forecast configuration (CONUS only) |
...
> **Note**: See [Measure Plugin](./plugins/measure.md) for detailed documentation on units and advanced usage.

### With Click-to-Forecast

Enable 48-hour NWS weather forecast on map click:
...
<Map
  ...
  eventHandlers={{
    onClickForecastFetched: (data) => {
      ...
      console.log('48hr rain:', data.hourly.reduce(
        (sum, h) => sum + (h.precipitationAmount ?? 0), 0
      ).toFixed(2), 'inches');
    },
  }}
/>
...
> **Note**: Click forecast only works for Continental US locations. See [Click Forecast Plugin](./plugins/click-forecast.md) for detailed documentation.
...
interface MeasureOptions {
  ...
  areaUnit?: 'metric' | 'imperial' | 'acres' | 'hectares';
}
```
...
```typescript
interface ClickForecastOptions {
  enabled?: boolean;
  position?: 'topleft' | 'topright' | 'bottomleft' | 'bottomright';
  forecastHours?: number;      // Default: 48, max: 168
  units?: 'imperial' | 'metric';
  popupMaxWidth?: number;      // Default: 400
  popupMaxHeight?: number;     // Default: 350
...
### Plugins
- [Click Forecast Plugin](./plugins/click-forecast.md) - 48-hour NWS weather forecast on click
- [Measure Plugin](./plugins/measure.md) - Distance and area measurement tools
